...
exports.getUserInfoByNickname = function(nickname,callback){
    model.findByNickname(nickname,function(err,result){
        ...
        var tmp = result;
        //查询数据库数据是否过期
        if(isNullObj(result) || (moment().valueOf() - result.LastUpdate) > 24 * 60 * 60 * 1000){
            var j = request.jar();
                ...
                jsdom.env(body,function(err,window){
                    ...
                    request({
                        ...
                        try{
                            ...
                            callback(err,body);
                            if(isNullObj(tmp)){
                                model.add(data,function(err){
                                    if(err){
                                        log.info('添加用户' + nickname + '数据失败\t');
                                    }else{
                                        log.info('添加用户' + nickname + '数据成功\t');
                                    }
                                });
                            }else{
                                data.id = tmp.id;
                                model.save(data,function(err){
                                    if(err){
